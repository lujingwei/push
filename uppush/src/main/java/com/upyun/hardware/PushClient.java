                ...
                AsyncRun.run(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (reconnectEnable && recTime <= 3) {
                            reconnect();
                            Toast.makeText(mContext, "网络或服务器异常正在重新连接...", Toast.LENGTH_SHORT).show();
                        } else {
                            mHandler.sendMessage(mHandler.obtainMessage(UConstant.MSG_NETWORK_ERROR));
                            Toast.makeText(mContext, "网络或服务器异常，请稍后再试", Toast.LENGTH_SHORT).show();
                            recTime = 0;
                        }
                    }
                }, 3000);
...
//            audioEncoder = new AudioEncoder(mSrsFlvMuxer);//音频先注释掉 关闭socket时shutdownOutput（）报错
